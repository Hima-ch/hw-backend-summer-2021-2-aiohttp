...
async def error_handling_middleware(request: "Request", handler):
    try:
        ...
        return error_json_response(http_status=400, status=HTTP_ERROR_CODES[400], message=e.reason, data=json.loads(e.text))


    except HTTPException as e:
        status_code = e.status
        error_status = HTTP_ERROR_CODES.get(status_code, HTTP_ERROR_CODES[500])
        return error_json_response(http_status=status_code, status=error_status, message=e.reason)

    except Exception as e:
        return error_json_response(http_status=403, status=HTTP_ERROR_CODES[403], message=str(e))
